...
public class GoodsSupplyController extends AdminController {
	...
	@Validate(groups = StoreAdminSubmit.class)
	public WebBean<Void> submit(@RequestBody GoodsSupplySubmitDTO dto) {
		dto.setSubmitSystem(SubmitSystem.STORE);
	...
	public WebBean<PageBean<GoodsSupplySimpleDTO>> page(GoodsSupplyQueryDTO dto) {
		StoreDTO store = storeService.getStore(getLoginAdmin().getStoreCode());
		dto.setCityId(String.valueOf(store.getCityId()));
		return WebBean.ok(goodsSupplyFacade.pageBySpu(dto));
	}

	@ApiOperation(value = "供应商品查询，单品")
	@RequestMapping(value = "page-single", method = RequestMethod.GET)
	@Validate
	public WebBean<PageBean<GoodsSupplyDTO>> pageSingle(GoodsSupplyQueryDTO dto) {
		dto.setMainSpu("0");
		return WebBean.ok(goodsSupplyFacade.page(dto));
	}
	...
	@ApiOperation(value = "审核成功多区域(上架)")
	@RequestMapping(value = "audit-success/permission", method = RequestMethod.POST)
	@Validate
	public WebBean<Void> auditSuccessPermission(@RequestBody GoodsSupplyAuditDTO dto) {
	...
	@ApiOperation(value = "审核失败多区域（驳回）")
	@RequestMapping(value = "audit-fail/permission", method = RequestMethod.POST)
	@Validate
	public WebBean<Void> auditFailPermission(@RequestBody GoodsSupplyAuditDTO dto) {
		dto.setAuditPermission(1);
		dto.setSubmitSystem(SubmitSystem.STORE);
		goodsSupplyFacade.auditFail(dto);
		return WebBean.ok();
	...
	@ApiOperation(value = "上架")
	@RequestMapping(value = "on-sale/permission", method = RequestMethod.POST)
	@Validate
	public WebBean<Void> onSalePermission(@RequestBody GoodsSupplyAuditDTO dto) {
		dto.setAuditPermission(1);
		dto.setSubmitSystem(SubmitSystem.STORE);
		goodsSupplyFacade.onSale(dto);
		return WebBean.ok();
	...
	@ApiOperation(value = "下架")
	@RequestMapping(value = "off-sale/permission", method = RequestMethod.POST)
	@Validate
	public WebBean<Void> offSalePermission(@RequestBody GoodsSupplyAuditDTO dto) {
		dto.setAuditPermission(1);
		dto.setSubmitSystem(SubmitSystem.STORE);
		goodsSupplyFacade.offSale(dto);
		return WebBean.ok();
