...
import com.morning.star.retail.admin.stock.controller.command.*;
...
public class ReceiptController extends AdminController {
    ...
    public WebBean<PageBean<ReceiptDTO>> list(ReceiptQueryDTO queryDTO) {
        ...
        queryDTO.setStoreCode(login.getStoreCode());
        PageBean<ReceiptDTO> returnVale = receiptFacade.list(queryDTO);
    ...
    public WebBean<Void> exportReceiptDetail(@ApiParam(value = "入库单号", required = true) @RequestParam String receiptCode, HttpServletResponse response) {
        ...
        try {
            new ExcelUtil<>(ReceiptItemExportDTO.class).writeToHttpResponse(list, "导出入库列表【" +
...
//        receiptAddDTO.setStoreCode(login.getStoreCode());
//        receiptAddDTO.setStoreName(login.getStoreName());
...
//        receiptItemAddDTO.setPrice(new BigDecimal(itemDTO.getPrice()));
        receiptItemAddDTO.setProductCode(goods.getProductCode());
