...
import com.morning.star.retail.admin.group.order.controller.command.AuditOrderCommand;
import com.morning.star.retail.admin.group.order.controller.command.ExamineGoodsCommand;
import com.morning.star.retail.admin.group.order.controller.command.QueryAfterSalesCommand;
import com.morning.star.retail.admin.group.order.controller.command.RefundOrderFormCommand;
...
import com.morning.star.retail.order.facade.dto.RefundOrderFormDTO;
...
public class AfterSalesController extends AdminController {
    ...
    public WebBean<PageBean<AfterSalesOrderDTO>> listAfterSalesOrders(QueryAfterSalesCommand search) {
        ...
        PageBean<AfterSalesOrderDTO> dataPage =
    ...
    @RequiresPermissions(value = "after-sales:part-refund")
    @ApiOperation(value = "申请部分退货")
    @RequestMapping(value = "/part-refund", method = RequestMethod.POST)
    @ResponseBody
    public WebBean<Void> portionRefund(@RequestBody RefundOrderFormCommand refundOrderFormCommand) {
        AdminLoginContent login = getLoginAdmin();
        refundOrderFormCommand.setStoreCode(login.getStoreCode());
        RefundOrderFormDTO refundOrderFormDTO = new RefundOrderFormDTO();
        BeanUtils.copy(refundOrderFormCommand, refundOrderFormDTO);
        facade.portionRefund(refundOrderFormDTO);
        return WebBean.ok();
    }
}