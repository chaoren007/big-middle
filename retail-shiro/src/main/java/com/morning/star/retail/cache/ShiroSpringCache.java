...
import com.morning.star.retail.shiro.util.SerializableUtil;
...
public class ShiroSpringCache<K, V> implements org.apache.shiro.cache.Cache<K, V> {
    ...
    public ShiroSpringCache(String name, CacheManager cacheManager, RedisTemplate<byte[], byte[]> redis) {
...
//        return (V) valueWrapper.get();
        ...
        byte[] valueByte = redis.opsForValue().get(wrapperKey(key).getBytes());
        if (valueByte == null) {
            return null;
        }
...
//        cache.put(wrapperKey(key), value);
        redis.opsForValue().set(wrapperKey(key).getBytes(), SerializableUtil.serialize(value));
        return get(key);
    }
    ...
    public V remove(K key) throws CacheException {
        V v = get(key);
        redis.delete(wrapperKey(key).getBytes());
